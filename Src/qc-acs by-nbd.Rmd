...
bg_counts %<>% mutate(tr_id=str_sub(bg_id, 1, 11))
glimpse(bg_counts)
```
...
```{r}
get_documentation <- function(vlist) {
  ...
  load_variables(2020, "acs5", cache = TRUE) %>%
    inner_join(data.frame(name=vlist), by="name")
}

vlist <- c("B25001_001", "B25002_001", "B25002_002", "B25002_003")
get_documentation(vlist)
```

```{r}
pull_acs <- function(vlist) {
  # This function takes a list of ACS
  # variable names and pulls the data
  # from ACS
  
  # get acs for each county

  co_list <- data.frame(
    co=c(37, 47, 95, 165, 91, 103, 209),
    st=rep(c(29, 20), c(4, 3))
  )  
...
acs_vlist <- pull_acs(vlist)
```

```{r}
reweight_acs_counts <- function(weight_data, acs_vlist) {
  # to merge properly, weight_data needs a 
  # comparable GEOID. For tracts, that is 
  # an 11 digit code. For block_groups,
  # that is a 12 digit code.
  
  # This program reweights any ACS data that 
  # represents a count of people or housing
  # units meeting a certain criteria.
  #
  # If the ACS data represents a mean, median,
  # or proportion, a slightly different approach
  # is needed.
  
  acs_vlist %>%
    inner_join(weight_data, by=c("GEOID")) %>%
    arrange(NID, GEOID, variable) %>%
    mutate(unit_weight=units_in/units) %>%
    mutate(estimate=estimate*w) %>%
...
bg_counts %>% 
  rename(GEOID=bg_id) %>%
  mutate(w=units_in/units) %>%
  reweight_acs_counts(acs_vlist)
```
