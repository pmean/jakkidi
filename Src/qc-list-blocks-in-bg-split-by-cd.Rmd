...
calc_sf_text <- function(sf_subset, sf_text) {
  sf_points <- st_point_on_surface(sf_subset)
  sf_coords <- as.data.frame(st_coordinates(sf_points))
  sf_coords$sf_text <- sf_text
  sf_coords
...
plot_green <- function(sf_subset, ti=" ") {
...
overlay_red_borders <- function(cd_map, bg_subset) {
  cd_map                               +
    geom_sf(
      data=bg_subset,
      aes(),
...
for (i_cd in interesting_cd[1:3]) {
  ...
  bg_coords <- calc_sf_text(
    bg_subset,
    str_sub(bg_subset$bg_id, 6, 12))
  print(bg_coords)
  ...
  plot_green(
      cd_subset, 
      ti=cd_subset$cd_name)            %>%
    overlay_bg_text(bg_coords)         %>%
    overlay_red_borders(bg_subset)     -> i_map
  plot(i_map)
  ...
  for (i_bg in bg_list$bg_id) {
    ...
    bl                                %>%
      filter(bg_id==i_bg)             %>%
      mutate(
        ab_id=str_sub(bl_id, 13, 15))  -> bl_subset

    bl_coords <- calc_sf_text(
      bl_subset,
      bl_subset$ab_id)
    print(bl_coords)
    ...
    plot_green(
      intersect_subsets,
      ti=i_bg)                        %>%
      overlay_red_borders(bl_subset)   -> m_map
    plot(m_map)
    m_map                             %>%
      overlay_bg_text(bl_coords)       -> m_map
    plot(m_map)
  }
}
```
